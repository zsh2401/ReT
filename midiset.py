...
def dataset_from(path: str, batch_size=64, max_seq_len=2048,ddp=False):
    ...
    if ddp:
        ...
        dataloader = DataLoader(dataset, sampler=sampler,pin_memory=True, drop_last=True, collate_fn=collator, batch_size=batch_size)
        return dataset, dataloader, len(tokenizer), tokenizer.pad_token_id,sampler
    else:
