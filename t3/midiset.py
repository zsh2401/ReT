...
def random_pick(
    ...
    # 按比例计算分割点
    total_count = int(len(paths) * entire_ratio)
...
def dataset_from(path: str,
    ...
    train_files, val_files, test_files = random_pick(files_paths,train_ratio,val_ratio,test_ratio,entire_ratio)

    train_dataset = MDataset(train_files,tokenizer,seq_len=max_seq_len)
    test_dataset = MDataset(test_files, tokenizer,seq_len=max_seq_len)
    val_dataset = MDataset(val_files, tokenizer,seq_len=max_seq_len)
