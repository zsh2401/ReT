...
def main():
    ...
    dff = args.dff
    seq_len = args.seq_len  # 填充后的最大序列长度
    ...
    (
        ...
        path=args.dataset, max_seq_len=seq_len, batch_size=batch_size, ddp=args.local_rank != -1,
        entire_ratio=args.dataset_scale
    )
    ...
    def train(epoch: int):
        ...
        # print("Updating epoch",train_sampler)
        ...
        with tqdm.tqdm(
            ...
            while True:
                ...
                bar.set_description(f"[{train_code}][{epoch}/{num_epochs}] Training, batch loss={loss.item():.5f}")
        ...
        with torch.no_grad():
            with tqdm.tqdm(
                ...
                while True:
                    try:
                        # X,Y,X_mask,Y_mask: BatchSize, SeqLen
                        X, Y, X_mask, Y_mask = next(val_iter)
                    except StopIteration:
                        break
                    except torch.OutOfMemoryError as e:
                        raise e
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.6f}, Avg Position {train_dataset.avg_position():.4f}")
        ppl, val_loss, bleu = validate(epoch)
        print(f"Epoch {epoch + 1}/{num_epochs}, PPL:{ppl:.6f}, Val Loss: {val_loss:.6f}, Bleu: {bleu:.6f}, AvgPosition: {val_dataset.avg_position()}")
        
        train_losses.append(train_loss)
        val_losses.append(val_loss)
        val_ppls.append(ppl)
        
        report(train_losses,val_losses,val_ppls)
        ...
        torch.save(
            ...
            f"checkpoints/{train_code}-{epoch + 1}-{val_loss:.4f}.pt",
        )

def report(train_losses: list[float],val_losses: list[float],val_ppls: list[float]):
    tl10 = train_losses[-10:]
    train_losses_10 = sum(tl10) / len(tl10)
    
    vl10 = val_losses[-10:]
    val_losses_10 = sum(vl10) / len(vl10)
    
    vp10 = val_ppls[-10:]
    val_ppls_10 = sum(vp10) / len(vp10)
    
    print(f"{train_losses_10=:.6f}, {val_losses_10=:.6f}, {val_ppls_10=:.6f}")
